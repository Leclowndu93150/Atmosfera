...
public record DimensionModifier(ResourceLocation id) implements AtmosphericSoundModifier, AtmosphericSoundModifier.Factory {
    @Override
    public float getModifier(EnvironmentContext context) {
        return Objects.requireNonNull(Minecraft.getInstance().level).dimension().location().equals(id) ? 1 : 0;
    }
    ...
    public static Factory create(JsonObject object) {
        return new DimensionModifier(ResourceLocation.parse(object.get("id").getAsString()));
    }
}
