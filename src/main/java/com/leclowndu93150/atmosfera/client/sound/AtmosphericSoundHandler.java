...
public class AtmosphericSoundHandler {
    ...
    private static final Map<ResourceLocation, Music> MUSIC_CACHE = new HashMap<>();
    ...
    private static ImmutableList<AtmosphericSound> getSoundsFromDefinitions(Map<ResourceLocation, AtmosphericSoundDefinition> definitions, ClientLevel world) {
    ...
    public void tick() {
        ...
        for (var sound : sounds) {
            if (tickingSounds.stream()
                    ...
                    .anyMatch(s -> sound.soundId().equals(s.getLocation())))
                continue;
    ...
    public Music getMusicSound(Music original) {
        ...
        SoundManager soundManager = client.getSoundManager();
        WeighedSoundEvents originalEvents = soundManager.getSoundEvent(original.getEvent().value().getLocation());
